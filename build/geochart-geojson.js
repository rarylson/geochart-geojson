...
(function (context) {
  ...
  function deepMerge_(obj1, obj2) {
    Object.entries(obj2).forEach(function (p) {
      if (p[0] in obj1 && _typeof(obj1[p[0]]) === "object" && obj1[p[0]] !== null && _typeof(p[1]) === "object") {
  ...
  GeoChart.prototype.draw = function (data) {
    ...
    this.map_.data.loadGeoJson(this.options_.geoJson, this.options_.geoJsonOptions, function (features) {
      ...
      this.map_.controls[google.maps.ControlPosition[this.options_.legend.position]].push(this.color_axis_.getContainer());
    ...
    this.map_.data.setStyle(function (feature) {
      ...
      if (feature.getProperty("data-value") !== undefined) {
        ...
        var gradient_colors_arr = [this.getColorArray_(this.options_.colorAxis.colors[0]), this.getColorArray_(this.options_.colorAxis.colors[1])];
        var gradient_stroke_colors_arr = [this.getColorArray_(this.options_.colorAxis.strokeColors[0]), this.getColorArray_(this.options_.colorAxis.strokeColors[1])];
        var relative_value = this.getRelativeValue_(feature.getProperty("data-value"));

        for (var i = 0; i < 3; i++) {
          fill_color_arr[i] = Math.round((gradient_colors_arr[1][i] - gradient_colors_arr[0][i]) * relative_value + gradient_colors_arr[0][i]);
          stroke_color_arr[i] = Math.round((gradient_stroke_colors_arr[1][i] - gradient_stroke_colors_arr[0][i]) * relative_value + gradient_stroke_colors_arr[0][i]);
        }
  ...
  ColorAxis.prototype.draw_ = function () {
    ...
    Object.assign(div_inner.style, { marginTop: -COLOR_AXIS_INDICATOR_TOP_OFFSET + "px" }, processTextStyle_(this.geo_chart_.options_.legend.textStyle));
    ...
    axis_div_inner.setAttribute("style", axis_div_inner.getAttribute("style") + "; " + this.getGradientStr_());
  ...
  ColorAxis.prototype.getGradientStr_ = function () {
    ...
    var gradient_colors_arr = [this.geo_chart_.getColorArray_(this.geo_chart_.options_.colorAxis.colors[0]), this.geo_chart_.getColorArray_(this.geo_chart_.options_.colorAxis.colors[1])];
    var gradient_colors_str = [this.geo_chart_.getColorArrayStr_(gradient_colors_arr[0]), this.geo_chart_.getColorArrayStr_(gradient_colors_arr[1])];
    gradient_string = gradient_string.replace(/\{c1\}/g, gradient_colors_str[0]).replace(/\{c2\}/g, gradient_colors_str[1]);
  ...
  ColorAxis.prototype.drawIndicator = function (feature) {
    ...
    this.indicator_span_.style.left = relative_value * width + COLOR_AXIS_INDICATOR_LEFT_OFFSET + "px";
